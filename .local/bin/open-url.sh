...
current_ff_cmd='hyprctl clients -j | jq ".[] | select(.initialClass == \"librewolf\" and .workspace.id == '$workspace_id') | .address"'
...
if [ -n "$current_ff_address" ]; then
    ...
    ff_address=$(hyprctl clients -j | jq -r '.[] | select(.initialClass == "librewolf") | .address' | head -n 1)
